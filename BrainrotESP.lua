...
function module.setup(opts)
	...
	local function applyTheme(newTheme)
		...
		for _, visuals in pairs(activeBrainrotVisuals) do
			-- Update Highlight Fill
			if visuals.hl then
				visuals.hl.FillColor = THEME.panel2 
			end
			-- Update SelectionBox Outline (The Fix)
			if visuals.box then
				visuals.box.Color3 = THEME.accentA
			end
	...
	local function createOrUpdateBrainrotVisuals(info)
		...
		if not visuals then
			...
			-- [[ 1. Highlight (Fill Only) ]]
			visuals.hl = Instance.new('Highlight')
			visuals.hl.FillColor = THEME.panel2 
			visuals.hl.FillTransparency = 0.5 
			visuals.hl.OutlineTransparency = 1 -- Hide Highlight outline
			...
			-- [[ 2. SelectionBox (Forced Outline) ]]
			visuals.box = Instance.new("SelectionBox")
			visuals.box.Name = "ForceOutline"
			visuals.box.Adornee = info.model or info.root
			visuals.box.LineThickness = 0.05
			visuals.box.Color3 = THEME.accentA -- Matches Border Theme
			visuals.box.SurfaceColor3 = THEME.accentA
			visuals.box.Transparency = 0.1
			visuals.box.SurfaceTransparency = 1 -- Hide surface, show lines
			visuals.box.Parent = info.model or info.root
		...
		-- Update SelectionBox Adornee
		if visuals.box then
			visuals.box.Adornee = info.model or info.root or target
			visuals.box.Parent = info.model or target or info.root
			visuals.box.Color3 = THEME.accentA
		end
	end
