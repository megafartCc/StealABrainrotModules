...
function module.setup(opts)
	...
	local function applyTheme(newTheme)
		...
		for _, visuals in pairs(activeBrainrotVisuals) do
			if visuals.hl then
				-- UPDATED COLOR LOGIC: Fill matches Background, Outline matches Border
				visuals.hl.FillColor = THEME.panel2 
				visuals.hl.OutlineColor = THEME.accentA
			end
	...
	local function createOrUpdateBrainrotVisuals(info)
		...
		if not visuals then
			...
			-- [[ 1. Create the Highlight Object (The Filler) ]]
			visuals.hl = Instance.new('Highlight')
			
			-- [[ 2. Set the Inner Fill (The Glow) ]]
			-- Matches THEME.panel2 (Billboard Background)
			visuals.hl.FillColor = THEME.panel2 
			visuals.hl.FillTransparency = 0.5 

			-- [[ 3. Set the Outer Outline (The Border) ]]
			-- Matches THEME.accentA (Billboard Border)
			visuals.hl.OutlineColor = THEME.accentA
			visuals.hl.OutlineTransparency = 0.1

			-- [[ 4. THE MAGIC LOGIC (See Through Walls) ]]
